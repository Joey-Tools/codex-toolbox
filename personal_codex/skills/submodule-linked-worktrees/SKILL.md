...
- The helper holds the exact target and parent directory objects through the Git write and creates new worktrees with `--no-checkout`. Registration, content checkout, final target/common-gitdir/HEAD/source-policy/object-closure validation, and shared-prefix receipt publication form one transaction. A failure removes the exact registered worktree, verifies registry and target absence, and then removes only still-matching transaction-created parent directories.
