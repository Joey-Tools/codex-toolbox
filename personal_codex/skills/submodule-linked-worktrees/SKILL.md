...
- Exact selected siblings may reuse a missing strict ancestor only after one participant publishes the same descriptor-created directory identity as a plan-owned receipt. Final worktree roots never enter the sibling receipt pool. A recursive parent instead publishes its own post-validation root identity under its plan-entry index before closing the lease, and every descendant must traverse that exact object. Replacement, access-policy drift, or name-semantics drift remains an error.
