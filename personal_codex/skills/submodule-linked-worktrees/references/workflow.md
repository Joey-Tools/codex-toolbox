...
For an apply invocation, the helper securely binds the top-level `.gitmodules`, the primary and any shared superproject index file objects/bytes, and the selected stage-0 gitlink rows before preflighting every selected target, including recursive descendants. The shared-index query deliberately preserves existing split-index state; applying the ordinary `core.splitIndex=false` write-suppression override to `rev-parse --shared-index-path` would hide the `link`-named file that the receipt must bind. The receipt is revalidated before each authorized fetch and immediately before the first worktree mutation. A path replacement, byte-content change, access-policy change, shared-index path-set change, or selected-row drift blocks the plan; an mtime-only transition does not. A failure in that phase prevents the apply phase from starting. `--dry-run` stops after the plan/preflight. When the task already names the exact paths and the preflight succeeds, the apply phase does not need a separate confirmation solely because the plan was printed.

For every existing managed worktree, the preflight binds its detached HEAD and semantic index snapshot, descriptor-reads the raw admin index, verifies its checksum and exact stage-0 semantics against HEAD, and rejects split-index plus hidden or unknown extensions before the no-update `read-tree` probe can run. It derives the bounded `HEAD..target` write set with renames disabled and binds the identity and access policy of each existing object and parent needed by that write set. It checks only write-relevant ignored prefixes and descendants rather than enumerating unrelated untracked content. Final revalidation repeats the descriptor-bound raw-index proof before the actual checkout, which uses `--no-overwrite-ignore --no-recurse-submodules`. The selected property is supported raw-index semantics; stat-cache timestamp churn is not treated as a checkout mutation.
