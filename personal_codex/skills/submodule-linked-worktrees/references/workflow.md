...
Registration remains provisional through every final target identity, common-gitdir, HEAD, source access/completeness/admission, checkout-closure, and shared-prefix receipt check. A failure closes the control receipt, removes the exact registered worktree through the held target-parent descriptor, verifies registry and target absence, and removes only still-matching transaction-created parent directories. Existing managed targets skip registration and use the same explicit bound admin/common control plane and checkout/post-validation from their held target descriptor. A malicious same-UID process that replaces a source/admin path in the residual interval after the child identity check but before stock Git opens that absolute path is outside this portable helper's prevention guarantee; postvalidation still detects it, but POSIX/macOS expose no directory-fd Git interface that can make that interval linearizable. The conceptual registration operation, with the process cwd already anchored to the held target directory, is:
