...
- Managed and new content checkout now consume an owner-private common-gitdir view containing descriptor-captured source config and common `info/attributes` bytes or absence, while `GIT_OBJECT_DIRECTORY` names the receipt-bound source objects. Source and private-view directory/file/absence leases all enter the child pre-exec gate; sparse checkout is forced off, unsupported repository-format extensions such as `refStorage` fail closed, and the permanent worktree `commondir` remains unchanged.
- Managed plan-only and `--dry-run` preflight now give tracked `git status` and `read-tree --dry-run` their own short-lived copy of that owner-private config/attributes view. Both conversion-capable probes explicitly name the managed worktree, consume only the bound source object directory, and carry every source/private directory, file, and absence lease into the child pre-exec gate; a launch-window `filter.clean`/`filter.process` plus `info/attributes` insertion therefore fails before Git starts and cannot defer detection until a later drift check.
